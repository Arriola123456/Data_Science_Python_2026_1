...
> **📺 Pantalla:** VSCode con el árbol de carpetas expandido. Apunta con el cursor a `src/`, después a `app/`, `data/`, `outputs/`, `docs/`.

> **🎙 Narración:**
>
> "El repositorio se organiza en cinco carpetas. `src/` contiene los cinco módulos del pipeline. `app/` es la app Streamlit. Los datos crudos descargados se guardan en `data/raw/` — gitignorados — y los limpios en `data/processed/` como parquet. Todas las salidas — seis figuras, cinco mapas y tres tablas CSV — quedan en `outputs/`. La metodología detallada vive en `docs/methodology.md`, y la guía de branches y FAQ del proyecto está en el README."
...
> **📺 Pantalla:** vuelve a VSCode y abre brevemente `src/ingest.py`, `src/geo_integration.py`, `src/metrics.py`. Después abre la terminal con el comando `python -m src.ingest` (no hace falta correrlo de nuevo, solo mostrarlo).

> **🎙 Narración:**
>
> "El pipeline son cinco scripts en `src/` que corren en orden. **`ingest.py`** descarga los cuatro datasets — MINSA, SUSALUD, INEI y el repo del curso — y los limpia: arregla un bug del MINSA donde las columnas NORTE y ESTE están **invertidas**, parsea el marcador de anonimización `NE_0001` de SUSALUD como nulo, y deriva el UBIGEO desde el campo `CÓDIGO` del INEI. Después **`geo_integration.py`** hace los GeoDataFrames, reproyecta a UTM 18S, y con un **KDTree** calcula la distancia desde cada uno de los 136 mil centros poblados al IPRESS de emergencia más cercano. **`metrics.py`** calcula el índice; **`viz_static.py`** y **`viz_geospatial.py`** generan figuras y mapas."
...
> **🎙 Narración:**
>
> "La app Streamlit es el entregable interactivo. Se levanta con `streamlit run app/streamlit_app.py` y tiene **cuatro tabs**.
>
...
- [ ] VSCode abierto en el repo con el árbol de `src/` expandido (para el bloque 4)
...
1. **El repo se organiza en `src/` (pipeline), `app/` (Streamlit), `data/`, `outputs/`, `docs/`.**
2. **El índice de cobertura vive en `[0, 1]`** y combina oferta + actividad + acceso, normalizadas con min-max.
3. **El pipeline son 5 scripts en orden:** ingest → geo_integration → metrics → viz_static → viz_geospatial.
